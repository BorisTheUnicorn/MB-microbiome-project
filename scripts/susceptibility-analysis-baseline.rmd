...
if (nsamples(ps_baseline) > 0) {
  current_cg_levels <- levels(as.factor(sample_data(ps_baseline)$carriage_group))
  if ("Non_carrier" %in% current_cg_levels && "Acquisition_carrier" %in% current_cg_levels) {
    sample_data(ps_baseline)$carriage_group <- factor(sample_data(ps_baseline)$carriage_group, 
                                                     levels = c("Non_carrier", "Acquisition_carrier"))
    message("Re-leveled carriage_group in ps_baseline. New reference: ", levels(sample_data(ps_baseline)$carriage_group)[1])
  } else {
    warning("Could not re-level carriage_group: one or both expected levels not found.")
  }
}
...
```{r run-ancombc2-baseline-models}
