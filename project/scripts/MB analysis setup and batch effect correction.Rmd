...
demographic_data <- read_excel("demographic data _Jan2022_final.YM.20241230.xlsx")
...
# Merge demographic data with sample table
dataset$sample_table <- dataset$sample_table %>%
...
# Check if SampleID values match otu_table column names
cat("Number of SampleID values that match OTU table column names:", 
    sum(sample_data$SampleID %in% colnames(otu_table)), "\n")

# Set row names of sample_data to be the values in SampleID column
rownames(sample_data) <- sample_data$SampleID

# Ensure all sample names in OTU table exist in sample_data
common_samples <- intersect(colnames(otu_table), rownames(sample_data))
...
# Create phyloseq object with corrected sample names
ps <- phyloseq(
...
# Verify the phyloseq object was created correctly
cat("Successfully created phyloseq object with", nsamples(ps), "samples\n")
...
# Compare microbiomes between carriers who clear NM vs those who remain persistent carriers
ps_resilience <- subset_samples(ps, 
                               carriage_group %in% c("Clearance_carrier", "Persistent_carrier"))

# Check sample sizes for resilience analysis
print("Sample counts for resilience analysis:")
print(table(sample_data(ps_resilience)$carriage_group, 
            sample_data(ps_resilience)$time))

# Run ANCOM-BC2 for resilience, including important covariates
set.seed(123)  # For reproducibility
ancombc2_resilience <- ancombc2(
  data = ps_resilience,
  tax_level = "Genus",
  fix_formula = "carriage_group + time + boardingschool + selfsmoking + AB_month",
  rand_formula = NULL,
  p_adj_method = "holm",
  pseudo_sens = TRUE,
  prv_cut = 0.10,
...
# Save results
saveRDS(ancombc2_resilience, "ancombc2_resilience_results.rds")

# 3. TEMPORAL PATTERNS ANALYSIS
# Examine interaction between time and carriage groups to identify taxa that change differently
# over time depending on carriage status
set.seed(123)  # For reproducibility
ancombc2_temporal <- ancombc2(
  data = ps,
  tax_level = "Genus",
  fix_formula = "carriage_group * time + boardingschool + selfsmoking + AB_month",
  rand_formula = NULL,
...
# Save results
saveRDS(ancombc2_temporal, "ancombc2_temporal_results.rds")
...
resilience_vis <- visualize_ancombc2_results(ancombc2_resilience, "Resilience Analysis")
temporal_vis <- visualize_ancombc2_results(ancombc2_temporal, "Temporal Patterns Analysis")

# Create a function to display results neatly
...
display_results(resilience_vis, "SIGNIFICANT TAXA FOR RESILIENCE ANALYSIS")
display_results(temporal_vis, "SIGNIFICANT TAXA FOR TEMPORAL PATTERNS ANALYSIS")

# Save plots as PDF if there are significant results
if (length(suscept_vis$plots) > 0) {
  pdf("susceptibility_significant_taxa.pdf", width = 10, height = 8)
  for (plot in suscept_vis$plots) {
    print(plot)
  }
  dev.off()
}

if (length(resilience_vis$plots) > 0) {
  pdf("resilience_significant_taxa.pdf", width = 10, height = 8)
  for (plot in resilience_vis$plots) {
    print(plot)
  }
  dev.off()
}

if (length(temporal_vis$plots) > 0) {
  pdf("temporal_significant_taxa.pdf", width = 10, height = 8)
  for (plot in temporal_vis$plots) {
    print(plot)
  }
  dev.off()
}
